...
export const calculatePlanetHealth = (
  ...
  // 1. Logistics Score (25%) - Tasks, Inventory, and Challenges
  const taskCompletion = tasks.length > 0 
    ? (tasks.filter(t => t.status === 'completed').length / tasks.length) * 100 
    : 100;
  const inventoryHealth = inventory.length > 0
    ? (inventory.filter(i => i.status === 'ok').length / inventory.length) * 100
    : 100;
  ...
  // 2. Finance Score (25%)
  const financeScore = 85; // Placeholder

  // 3. Spiritual/Bond Score (25%) - Worship, Romance, and Library
