...
export const VitalSignsLog: React.FC = () => {
  const { vitals, currentUser } = useKokab();
  ...
  return (
    ...
    >
      <div className="space-y-2">
        ...
        <p className="text-sm text-[var(--color-text-secondary)]">مزامنة المؤشرات الحيوية والنشاط البدني</p>
      </div>
      ...
      <div className="glass-card p-4 flex items-center justify-between bg-[var(--color-bg-surface)]/30">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest">متصل بـ Google Fit</span>
        </div>
        <span className="text-[10px] opacity-50">آخر مزامنة: منذ دقيقتين</span>
      </div>
      ...
      {partnerVitals.sleepQuality < 50 && (
        <section className="p-6 rounded-2xl bg-amber-500/10 border border-amber-500/20 space-y-4">
          ...
          <div className="flex gap-2">
            <button className="flex-1 py-2 bg-amber-500 text-white rounded-lg text-[10px] font-bold flex items-center justify-center gap-2">
